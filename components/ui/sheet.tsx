...
>(({ className, ...props }, ref) => (
  <SheetPrimitive.Overlay
    className={cn(
      // Same touch guard as the Dialog overlay and the sticky navbar: a fixed
      // backdrop-filter forces a full repaint every frame the mobile browser
      // chrome animates. This overlay backs the cart drawer, mobile menu and
      // filter sheet, so it's the highest-traffic instance of that cost.
...
>(({ side = "right", className, children, ...props }, ref) => (
  <SheetPortal>
    ...
    <SheetPrimitive.Content
      ...
      {...props}
    >
      <SheetPrimitive.Close className="absolute right-5 top-5 opacity-70 transition-opacity hover:opacity-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2">
        <X className="h-5 w-5" />
